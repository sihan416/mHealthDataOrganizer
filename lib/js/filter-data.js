...
function clickHandler() {
  ...
  var wdescription = document.getElementById("weightDescription").checked;
  ...
  if (fileSelector.files.length > 0 && fileSelector.files[0].type == "application/json"){
    ...
    fileSelector.files[0].text().then((txt)=>{
      try{
        ...
        else if (document.getElementById('data-type').value == "weight"){
          if (Array.isArray(parsed)){
            parsed.forEach((element) => {
              var valid = true;
              if (!element.body_weight){
                valid = false;
              }
              if (wdescription && !element.descriptive_statistic) {
                valid = false;
              }
              if (only && element.descriptive_statistic) {
                valid = false;
              }
              if (valid){
                validEntries.push(element);
...
function dataTypeChange() {
  ...
  var Welement = document.getElementById("weight-options");
